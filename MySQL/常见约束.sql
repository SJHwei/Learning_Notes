...
2、添加表级约束：alter table add 【constraint 约束名】 约束类型(字段名) 【外键的引用】；
...
ALTER TABLE emp2 MODIFY COLUMN id INT PRIMARY KEY; #列级约束不支持起名字，表级约束支持起名字
ALTER TABLE emp2 ADD CONSTRAINT my_emp_id_pk PRIMARY KEY(id);
#2.向表dept2的id列中添加primary key约束
ALTER TABLE dept2 MODIFY COLUMN id INT PRIMARY KEY;
#3.向表emp2中添加dept_id，并在其中定义foreign key约束，与之相关联的列式dept2表中的id列(fk_emp2_dept2)。
